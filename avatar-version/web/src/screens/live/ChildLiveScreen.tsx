...
import duckAvatarSprite from '../../assets/avatar/duck-avatar-sprite.png';
...
export function ChildLiveScreen({ mode }: { mode: Mode }) {
  ...
  return (
    <div className="flex min-h-full flex-1 flex-col bg-paper">
      <header className="mb-5 flex items-center gap-4 pt-2 sm:mb-10 sm:gap-6 sm:pt-8">
        <div className="grid size-24 shrink-0 place-items-center rounded-[2rem] border border-paper-line bg-paper-card/75 p-2 shadow-card sm:size-32 sm:rounded-[2.5rem] sm:p-3">
          <SpriteAvatar
            src={duckAvatarSprite}
            state={pttState}
            label={`${screenCopy.eyebrow} duck avatar`}
            size={128}
          />
        </div>
        <div className="min-w-0">
          <span className="text-xs font-bold uppercase tracking-[0.28em] text-accent">{screenCopy.eyebrow}</span>
          <h1 className="mt-2 font-serif text-[2.35rem] leading-none text-ink sm:mt-3 sm:text-7xl">{screenCopy.title}</h1>
